...
import {
  buildValorFormaPagamentoMatrix,
  type ValorFormaPagamentoFlatRow,
} from "@/lib/queries/comercial/valor-forma-pagamento-matrix-aggregate";
import type { ValorFormaPagamentoRow } from "@/lib/queries/comercial/valor-forma-pagamento-extract";
...
export type ValorFormaPagamentoDefaultPeriod = "currentMonth" | "previousMonth";

function defaultDateRange(which: ValorFormaPagamentoDefaultPeriod): {
  dataInicio: string;
  dataFim: string;
} {
...
type Props = {
  ...
  defaultPeriod?: ValorFormaPagamentoDefaultPeriod;
};

export function ValorFormaPagamentoMatrix({
  titulo,
  exportFilePrefix,
  autoLoad = false,
  lookups,
  ...
  const [rows, setRows] = useState<ValorFormaPagamentoRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const matrix = useMemo(() => buildValorFormaPagamentoMatrix(rows), [rows]);
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/relatorios/comercial/valor-forma-pagamento?${params.toString()}`, {
        priority: "high",
        cache: "no-store",
      } as RequestInit);
      const json = (await res.json()) as { rows?: ValorFormaPagamentoRow[]; error?: string };
      if (!res.ok) {
        setRows([]);
  ...
  const exportarXlsx = useCallback(async () => {
    ...
    for (const mk of matrix.monthKeys) {
      header.push(`${mk} — Valor (R$)`, `${mk} — %`);
    }
    header.push("Total — Valor (R$)", "Total — %");
    const data: (string | number)[][] = [header];
    for (const fr of matrix.flatRows) {
      const line: (string | number)[] = [String(fr.level), fr.label];
      fr.cells.forEach((c) => {
        line.push(Number(c.valor.toFixed(2)), Number(c.pct.toFixed(2)));
      });
      data.push(line);
    }
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Valor forma pagto");
  ...
  function rowClass(fr: ValorFormaPagamentoFlatRow, stripe: boolean): string {
    const base =
  ...
  function treeCellClass(fr: ValorFormaPagamentoFlatRow): string {
  ...
  return (
    <section className={styles.section}>
      <div className={styles.periodBlock}>
        ...
        <h4 className={styles.matrixSubtitle}>Valor líquido por forma de pagamento</h4>
        <p className={styles.filterHint} style={{ marginTop: 8 }}>
          Igual à aba de quantidade, mas a métrica é <strong>valor líquido</strong> (
          <code>tab_resumo_venda_item.val_liquido</code> com{" "}
          <code>seq_venda = seq_cupom</code>; se não houver resumo, <code>val_total_item</code> na linha do cupom).
          Subgrupo <code>1</code>, repartição por linhas de <code>tab_pagamento_cupom</code> no cupom.
        </p>
      </div>
              ...
              <>
                {matrix.flatRows.map((fr, idx) => {
                  ...
                  return (
                    <tr key={fr.key} className={rowClass(fr, stripe)}>
                      ...
                      {fr.cells.map((c, i) => (
                        <Fragment key={`c-${i}`}>
                          <td className={styles.numCell}>{fmtBrl(c.valor)}</td>
                ...
                <tr className={styles.rowTotal}>
                  ...
                  {matrix.columnTotals.map((v, i) => (
                    <Fragment key={`t-${i}`}>
                      <td className={styles.numCell}>{fmtBrl(v)}</td>
