...
export type ValorFormaPagamentoMesCell = {
  valor: number;
  pct: number;
};

export type ValorFormaPagamentoFlatRow = {
  key: string;
  level: 0 | 1 | 2;
  label: string;
  bold: boolean;
  cells: ValorFormaPagamentoMesCell[];
};
...
/** Hierarquia empresa → grupo → forma; `%` = valor da célula / total da coluna. */
export function buildValorFormaPagamentoMatrix(
  rows: ValorFormaPagamentoRow[]
): {
  monthKeys: string[];
  columnTotals: number[];
  flatRows: ValorFormaPagamentoFlatRow[];
} {
  type Agg = { valor: number };
  ...
  for (const r of rows) {
    ...
    const k = `${emp}|${mes}|${g}|${f}`;
    const v = coerceNum(r.valor_linha);
    const prev = leafMap.get(k);
    if (prev) prev.valor += v;
    else leafMap.set(k, { valor: v });
  }
  ...
  function cellVal(emp: string, mes: string, g: string, f: string): number {
    if (mes === "total") {
      let s = 0;
      for (const mk of monthKeys) {
        s += leafMap.get(`${emp}|${mk}|${g}|${f}`)?.valor ?? 0;
      }
      return s;
    }
    return leafMap.get(`${emp}|${mes}|${g}|${f}`)?.valor ?? 0;
  }
  ...
  function sumFormas(emp: string, mes: string, g: string): number {
    ...
    let s = 0;
    for (const fn of Array.from(fs)) s += cellVal(emp, mes, g, fn);
    return s;
  }
  ...
  function pct(val: number, colIdx: number): number {
    const den = columnTotals[colIdx] ?? 0;
    return den > 0 ? (val / den) * 100 : 0;
  }

  function cellsFor(emp: string, g: string | null, f: string | null): ValorFormaPagamentoMesCell[] {
    return colKeys.map((mk, i) => {
      let val = 0;
      if (f != null && g != null) {
        val = cellVal(emp, mk, g, f);
      } else if (g != null) {
        val = sumFormas(emp, mk, g);
      } else {
        val = sumGrupos(emp, mk);
      }
      return { valor: val, pct: pct(val, i) };
    });
  }
  ...
  const flatRows: ValorFormaPagamentoFlatRow[] = [];
