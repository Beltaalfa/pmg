...
import {
  fetchQuantidadeMargemProjecao,
  type QuantidadeMargemFiltros,
} from "@/lib/queries/comercial/quantidade-margem-projecao";

function apiCacheRevalidateSeconds(): number {
  const n = Number.parseInt(
    process.env.PMG_QUANTIDADE_MARGEM_PROJECAO_API_CACHE_SECONDS ??
      process.env.PMG_QUANTIDADE_MARGEM_API_CACHE_SECONDS ??
      "0",
    10
  );
    ...
    const rows =
      ttl > 0
        ? await unstable_cache(
            () => fetchQuantidadeMargemProjecao(filtros),
            [
              "quantidade-margem-projecao",
              filtros.dataInicio,
              filtros.dataFim,
        ...
        : await fetchQuantidadeMargemProjecao(filtros);
